...
public class MyraUI
{
	...
	public MyraUI(Desktop desktop) {
		...
		string data = File.ReadAllText(FileLocations.QML + "test.xmmp");
		...
		var imgtest = (Image)project.Root.FindWidgetById("label");
		imgtest.Renderable = MyraEnvironment.DefaultAssetManager.Load<TextureRegion>(FileLocations.QML + "LeftPanel12.png");
		...
		window1.Content.TouchDoubleClick += (s, a) => {
			...
			var myQml = new PictureQml(window1);
		};
		...
		//RootGrid.Widgets.Add(label);
		//label.MouseEntered += (s, a) => MouseIsOver = true;
		//label.MouseLeft += (s, a) => MouseIsOver = false;
		
		RootGrid.Widgets.Add(window1);
		window1.MouseEntered += (s, a) => MouseIsOver = true;
		window1.MouseLeft += (s, a) => MouseIsOver = false;

		RootGrid.Widgets.Add(painterModeButton);
		painterModeButton.MouseEntered += (s, a) => MouseIsOver = true;
		painterModeButton.MouseLeft += (s, a) => MouseIsOver = false;

		RootGrid.Widgets.Add(presentedColorSquare);
		presentedColorSquare.MouseEntered += (s, a) => MouseIsOver = true;
		presentedColorSquare.MouseLeft += (s, a) => MouseIsOver = false;

		RootGrid.Widgets.Add(textbox);
		textbox.MouseEntered += (s, a) => MouseIsOver = true;
		textbox.MouseLeft += (s, a) => MouseIsOver = false;	
		
		RootGrid.Widgets.Add(travelButton);
		travelButton.MouseEntered += (s, a) => MouseIsOver = true;
		travelButton.MouseLeft += (s, a) => MouseIsOver = false;

		/*RootGrid.Widgets.Add(_informationBox);
		_informationBox.MouseEntered += (s, a) => MouseIsOver = true;
		_informationBox.MouseLeft += (s, a) => MouseIsOver = false;#1#
	...
	public static void BuildUI()
		{
			/*void //TopOfUi( Widget widget) {
				widget.MouseEntered += (s, a) => MyraUI.MouseIsOver = true;
				widget.MouseLeft += (s, a) => MyraUI.MouseIsOver = false;
			}#2#
		...
		//	//TopOfUi(textButton4);
			...
			monthBox.MouseLeft += (s, a) => {
				/*for (int i = 1; i <= Months.Length; i++)
					if (monthBox.SelectedItem.Text == Months[i]) { Month = i; break;}#2# 
